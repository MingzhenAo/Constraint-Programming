...
'''
'''
        [               (),(313,213,223,312,214),                   (),               (),           (),           (),           (),(231,331,241,232),(123,122,222,133)],
...
while n<9:
    ...
    if not path.isdir('playingmode1/wizard/model'+str(n)):
        os.mkdir('playingmode1/wizard/model'+str(n))
    with open('playingmode1/wizard/model'+str(n)+'/'+name,"w") as f1:
        for line in f:
            f1.write(line);
        m=0
        while m<len(element):
            ...
            if len(element[m])!=0:
               ...
               for line in f:
                      f1.write(line);
            m+=1
    os.system("minizinc.exe -c --solver org.minizinc.mzn-fzn"+'  playingmode1/wizard/model'+str(n)+'/'+name+" --sac" )
    for solver in solvers:
            file = open('playingmode1/wizard/model'+str(n)+'/'+'model' + str(n) + solver+'.log', 'w')
            if solver=='picat':
                begin=time.time()
                command ="timeout 1800 picat fzn_picat_sat.pi playingmode1/wizard/model"+str(n)+'/model'+str(n)+".fzn"
            elif solver=='jacop':
                begin=time.time()
                command = "java -cp jacop-4.7.0.jar org.jacop.fz.Fz2jacop playingmode1/wizard/model"+str(n)+'/model'+str(n)+".fzn"
            elif solver=='choco':
                begin = time.time()
                command ="java -cp choco-parsers-4.10.3-jar-with-dependencies.jar org.chocosolver.parser.flatzinc.ChocoFZN playingmode1/wizard/model"+str(n)+'/model'+str(n)+".fzn"
            else:
                command = 'timeout 1800 minizinc.exe --solver ' + solver + " " + 'playingmode1/wizard/model'+str(n)+'/'+name + " --output-time --sac"
            try:
