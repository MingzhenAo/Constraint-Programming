...
'''
junior=[[(113,114,115,124),                   (),(231,331,321,322,241),               (),           (),(141,151,142),(213,214,223),(411,511,421,412),(232,132,133,222)],
        [               (),(331,321,322,231,421),(133,123,132,232,124),               (),(111,121,131),(114,115,214),(222,221,122),(141,142,241,151),               ()],
        ...
        [(311,312,313,412),(223,213,113,222,214),                   (),(142,132,133,123),           (),(411,511,421),(114,115,124),               (),               ()],
...
while n<9:
    ...
    if not path.isdir('playingmode1/junior/model'+str(n)):
        os.mkdir('playingmode1/junior/model'+str(n))
    with open('playingmode1/junior/model'+str(n)+'/'+name,"w") as f1:
        for line in f:
            f1.write(line);
        m=0
        while m<len(element):
            ...
            if len(element[m])!=0:
               ...
               for line in f:
                      f1.write(line);
            m+=1
    os.system("minizinc.exe -c --solver org.minizinc.mzn-fzn"+'  playingmode1/junior/model'+str(n)+'/'+name+" --sac" )
    for solver in solvers:
            file = open('playingmode1/junior/model'+str(n)+'/'+'model' + str(n) + solver+'.log', 'w')
            if solver=='picat':
                begin=time.time()
                command ="timeout 1800 picat fzn_picat_sat.pi playingmode1/junior/model"+str(n)+'/model'+str(n)+".fzn"
            elif solver=='jacop':
                begin=time.time()
                command = "java -cp jacop-4.7.0.jar org.jacop.fz.Fz2jacop playingmode1/junior/model"+str(n)+'/model'+str(n)+".fzn"
            elif solver=='choco':
                begin = time.time()
                command ="java -cp choco-parsers-4.10.3-jar-with-dependencies.jar org.chocosolver.parser.flatzinc.ChocoFZN playingmode1/junior/model"+str(n)+'/model'+str(n)+".fzn"
            else:
                command = 'timeout 1800 minizinc.exe --solver ' + solver + " " + 'playingmode1/junior/model'+str(n)+'/'+name + " --output-time --sac"
            try:
