...
'''
'''     
        [(311,411,511,412),(313,213,223,312,214),                   (),               (),           (),(321,421,322),           (),               (),(221,231,331,222)]
        [               (),                   (),(232,231,132,142,331),               (),           (),(312,313,322),           (),(222,223,122,212),(111,121,221,112)]
        [(113,114,115,124),(223,222,212,123,212),                   (),(312,313,213,214),           (),(132,133,142),           (),               (),               ()]
...
'''
master=[[               (),(313,213,223,312,214),                   (),               (),           (),           (),           (),(231,331,241,232),(123,122,222,133)],
        [(221,231,241,331),                   (),                   (),               (),           (),(141,151,142),           (),(132,122,131,232),               ()],
        [(122,123,124,133),                   (),                   (),(312,313,213,214),(113,114,115),           (),           (),               (),               ()],
        [               (),(142,141,241,132,151),                   (),(421,321,331,231),           (),           (),           (),(312,313,412,322),               ()],
        [(131,132,133,232),                   (),                   (),               (),           (),           (),           (),(141,142,241,151),(231,221,222,331)],
        [(122,132,142,133),                   (),                   (),(312,313,213,214),           (),(112,113,212),           (),               (),               ()],
        [               (),                   (),(231,331,321,322,241),               (),           (),(114,115,124),(213,214,313),               (),               ()],
        [               (),                   (),                   (),               (),           (),(114,115,124),           (),(312,313,412,322),(213,113,123,214)]]
...
while n<9:
    ...
    if not path.isdir('playingmode1/master/model'+str(n)):
        os.mkdir('playingmode1/master/model'+str(n))
    with open('playingmode1/master/model'+str(n)+'/'+name,"w") as f1:
        for line in f:
            f1.write(line);
        m=0
        while m<len(element):
            ...
            if len(element[m])!=0:
               ...
               for line in f:
                      f1.write(line);
            m+=1
    os.system("minizinc.exe -c --solver org.minizinc.mzn-fzn"+'  playingmode1/master/model'+str(n)+'/'+name+" --sac" )
    for solver in solvers:
            file = open('playingmode1/master/model'+str(n)+'/'+'model' + str(n) + solver+'.log', 'w')
            if solver=='picat':
                begin=time.time()
                command ="timeout 1800 picat fzn_picat_sat.pi playingmode1/master/model"+str(n)+'/model'+str(n)+".fzn"
            elif solver=='jacop':
                begin=time.time()
                command = "java -cp jacop-4.7.0.jar org.jacop.fz.Fz2jacop playingmode1/master/model"+str(n)+'/model'+str(n)+".fzn"
            elif solver=='choco':
                begin = time.time()
                command ="java -cp choco-parsers-4.10.3-jar-with-dependencies.jar org.chocosolver.parser.flatzinc.ChocoFZN playingmode1/master/model"+str(n)+'/model'+str(n)+".fzn"
            else:
                command = 'timeout 1800 minizinc.exe --solver ' + solver + " " + 'playingmode1/master/model'+str(n)+'/'+name + " --output-time --sac"
            try:
