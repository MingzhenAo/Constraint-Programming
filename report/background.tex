...
\end{lstlisting}
The contents in line 1 are notes, which must start from "\%". They provide explanations for reader to understand the codes. And the contents in line 2 aim to call "alldifferent.mzn", which contains "alldifferent" constraint. The 'include "alldifferent.mzn"' can be replaced by 'include "globals.mzn"' because it contains all global constraints. Accordingly, the contents in line 4 call the "alldifferent" constraint, which makes that there are no two variables equal to the same value. 
...
\end{lstlisting}
From line 10 to 11, some variables and corresponding types has been defined. As an example, "var position: Vb11;" means Vb11 is a variable, and the type of Vb11 is position. Therefore, the domain of Vb11 is the elements in position.
...
\end{lstlisting}
Accordingly, From line 15 to line 18, the function has been called in the constraint.
