...
\section{IQ Twist}
IQ Twist is a puzzle game that consists of 8 twisted puzzle pieces and 7 coloured pegs (Figure~\ref{fig:IQ_twist_game}). To play the game, the colored pegs should be placed on the board in advance, then fit all the twisted puzzle pieces back into the board. Different unit of piece have to match the pegs with the same color and only the hollow units of a piece can be used to match pegs. Moreover, the difficulty of a single game depends on the pegs' placement given at the beginning.
\begin{figure}[htbp]
    \centering
...
\begin{enumerate}
 ...
 \item $V_{y21}=(0,0)$, $V_{y22}=(-1,0)$, $V_{y23}=(-1,-1)$, $V_{y24}=(-2,-1)$, $V_{y25}=(-1,-2)$.
\end{enumerate}
In our case, the piece can be moved as long as all units of the piece are on the board. Both $x_{0}$ and $y_{0}$ can be variables, hence we get the general forms
\begin{enumerate}
...
\end{lstlisting}
\bigskip
Furthermore, the constraints will be separately addressed. The code in Listing~\ref{lst:IQ Twist constraint1} represents the first constraint, which means there are no two different units of piece in the same position.
...
\\For the third constraint, as an example for yellow2, it is encoded in Listing~\ref{lst:IQ Twist constraint3}.
...
\end{lstlisting}
\bigskip
In this constraint, because the positions are encoded as integers such as $(1,1)$ to 11. The g0 function is used to get the corresponding $x$ value and g1 function is used to get the corresponding $y$ value. Accordingly, the functions are represented in Listing~\ref{lst:IQ Twist functions}.
