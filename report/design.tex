...
Figure~\ref{fig:last_case} shows there are only 3 pegs on the board. On account of the coordinate system in Figure~\ref{fig:coordinate}, the positions of blue peg, red peg, and green peg are separately placed on (5,2), (6,1) and (5,4). And the other pegs are not on the board. Hence, they could be encoded in Listing~\ref{lst:pegs' positions}.
...
\end{lstlisting}
\bigskip
Secondly, the domain of \VUnits is defined by "position" that corresponds to Listing~\ref{lst:IQ Twist domain}. In such a case, 11 in "position" means the position (1,1), 21 means the position (2,1) and so on. However, the domain of \VPegs is just be set as the set of all integers in Listing~\ref{lst:IQ Twist domain}, it does not matter because the specific positions for pegs have been set in Listing~\ref{lst:pegs' positions}.
...
\end{lstlisting}
\bigskip
Furthermore, the constraints will be separately addressed. The codes in Listing~\ref{lst:IQ Twist constraint1} represent the first constraint, which means there is no 2 different unit variables contain the same value.
...
\\For the third constraint, as an example for yellow2, it could be encoded in Listing~\ref{lst:IQ Twist constraint3}.
...
\end{lstlisting}
\bigskip
In this constraint, g0 function is used to get the corresponding $x$ value and g1 function is used to get the corresponding $y$ value. Accordingly, the functions represented in Listing~\ref{lst:IQ Twist functions}.
...
\end{lstlisting}
\bigskip
Comparably, the other pieces can be encoded. For the last constraint, as an instance, the yellow peg1 can be encoded as below.
...
\begin{lstlisting}[language=minizinc,numbers=none,caption={Encoding for red piece1 of special problem},label={lst:red1}]
constraint (Vr11=422 \/  Vr12=422);
\end{lstlisting}
\bigskip
It means that we need to consider all possible situations.
\subsubsection{The General Model}
...
\\In addition, in the final codes, if the positions of the units of piece has been placed before the game start, the constraint does not need to appear again. As an example, for the last problem in Zig Zag Puzzler, the orange piece and blue piece1 have been set on the board, hence, the constraints about these two pieces will not appear in the final codes.
\\The complete codes for the last problem in Zig Zag Puzzler playing mode1 are in Appendix~\ref{appendix:lastcaseinmode1}, and the complete codes for the last problem in Zia Zag Puzzler playing mode2 are in Appendix~\ref{appendix:lastcaseinmode2}. 