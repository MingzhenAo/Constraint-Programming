...
\caption{Initial state of each piece}
  \label{fig:allinit}
\end{figure}
There are some name rules. Firstly, all the variables are corresponding to the specific units of pieces. As an example, for piece yellow2, Figure~\ref{fig:namerules} shows that the first variable $V_{y21}$ is used to represent the left and bottom unit. For other units, they are named as $V_{y22}$, $V_{y23}$, $V_{y24}$ and $V_{y25}$, which follows the order from left to right and bottom-up. Similarly, the name rules will be used for other pieces. 
\begin{figure}[htbp]
    \centering
    ...
    \label{fig:namerules}
\end{figure}
In addition, there are 7 pegs. Each of them is assigned a variable, the subscript represents its color such as $V_{py1}$ that $_{py1}$ means yellow peg1. Therefore, the variables can be represented as \VUnits and \VPegs,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Secondly, there should be no 2 different reg variables contain the same value except both of them are not on the board
\begin{equation}
\begin{aligned}
...
All the problems in the IQ Twist booklet are encoded in Minizinc.  Based on the CSP model which is mentioned above, to set a specific model only needs to add the position information for the pegs. 
\begin{figure}[htbp]
    \centering
    ...
    \label{fig:last_case}
\end{figure}
For example, with regard to the problem in Figure~\ref{fig:last_case}, it can be encoded by 2 parts, pegs' position information which is different in each problem and the general CSP model that is the same in all problems.
...
\end{lstlisting}
\bigskip
Secondly, the domain of \VUnits is defined by "position" that corresponds to Listing~\ref{lst:IQ Twist domain}. In such a case, 11 in "position" means the position (1,1), 21 means the position (2,1) and so on. However, the domain of \VPegs is all integers, it does not matter because the specific positions for pegs have been set in Listing~\ref{lst:pegs' positions}.
...
\end{lstlisting}
\bigskip
In this constraint, g0 function is used to get the corresponding $x$ value and g1 function is used to get the corresponding $y$ value. Accordingly, the functions are represented in Listing~\ref{lst:IQ Twist functions}.
...
\label{sec:3Ddomains}
Figure~\ref{fig:board1} and Figure~\ref{fig:board2} show how to create the coordinate systems for both modes to represent the positions on the boards. For both of them, similar to IQ Twist, the positions are represented as tuples and each tuple contains three elements which are integers. The position of origin of coordinate can be represented as $(1,1,1)$. Now, one position on the board is considered as $(x_{0},y_{0},z_{0})$.
\begin{figure}[htbp]
\centering
...
\end{aligned}
\end{equation}
If $(x_{0},y_{0},z_{0})$ is located in one of the three planes. Such as xy-plane which corresponds to the position $(x_{0},y_{0},1)$, it will satisfy
\begin{equation}
x_{0}+y_{0}\leq6.
\end{equation}
...
\label{fig:board2B}
\end{subfigure}
\caption{The board of ZIA ZAG Puzzler mode2}
  \label{fig:board2}
\end{figure}
For playing mode2, Figure~\ref{fig:board2} shows that the max number for both y-axis and z-axis are 4 and x-axis is 5, hence, 
\begin{equation}
\begin{aligned}
&0<x_{0}\leq5,\\
&0<y_{0}\leq4,\\
...
\end{aligned}
\end{equation}
always corresponds one combination among 
\begin{equation}
\begin{aligned}
&\beta=180^{\circ};\\
&\theta=0^{\circ}, 90^{\circ}, 180^{\circ}, 270^{\circ};\\
